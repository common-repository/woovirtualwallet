...
require_once LWS_ADMIN_PANEL_ASSETS.'/cssinliner/simple_html_dom/simple_html_dom.php';
require_once LWS_ADMIN_PANEL_ASSETS.'/cssinliner/csstidy/class.csstidy.php';
...
class CSSIN
{
	...
	*/
	...
	{
			$css  = new \LWS\Adminpanel\Assets\csstidy();
			$css->parse($text);
	...
	*/
	function inlineCSS($url, $contents=null)
	{
		...
		if($contents === null)
		{
			$html = \LWS\Adminpanel\Assets\file_get_html($url, false, null, -1, -1, true, true, LWS_HDOM_DEFAULT_TARGET_CHARSET, false, LWS_HDOM_DEFAULT_BR_TEXT, LWS_HDOM_DEFAULT_SPAN_TEXT);
		}
		// Else use the data provided!
		else
		{
			$html = \LWS\Adminpanel\Assets\str_get_html($contents, true, true, LWS_HDOM_DEFAULT_TARGET_CHARSET, false, LWS_HDOM_DEFAULT_BR_TEXT, LWS_HDOM_DEFAULT_SPAN_TEXT);
		}
		...
		{
			foreach($html->find($rule['selector'], null, true) as $node)
			{
