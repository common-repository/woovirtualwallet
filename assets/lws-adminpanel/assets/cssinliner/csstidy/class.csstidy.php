 ...
 */
define('LWS_CSSTIDY_AT_START',         1);
define('LWS_CSSTIDY_AT_END',           2);
define('LWS_CSSTIDY_SEL_START',        3);
define('LWS_CSSTIDY_SEL_END',          4);
define('LWS_CSSTIDY_PROPERTY',         5);
define('LWS_CSSTIDY_VALUE',            6);
define('LWS_CSSTIDY_COMMENT',          7);
define('LWS_CSSTIDY_IMPORTANT_COMMENT',8);
define('LWS_CSSTIDY_DEFAULT_AT',      41);
...
class csstidy {
	...
	public function _add_token($type, $data, $do = false) {
		if ($this->get_cfg('preserve_css') || $do) {
			...
			if ($type === LWS_CSSTIDY_AT_START
				and count($this->tokens)
				and $last = end($this->tokens)
				and $last[0] === LWS_CSSTIDY_AT_END
				and $last[1] === trim($data)) {
				array_pop($this->tokens);
			}
			else {
				$this->tokens[] = array($type, ($type == LWS_CSSTIDY_COMMENT or $type == LWS_CSSTIDY_IMPORTANT_COMMENT) ? $data : trim($data));
			}
		}
	}
				...
				case 'at':
					if ($this->is_token($string, $i)) {
						if ($string[$i] === '/' && @$string[$i + 1] === '*') {
							...
							$this->_add_token(LWS_CSSTIDY_AT_START, $this->at);
						} elseif ($string[$i] === ',') {
							$cur_at = trim($cur_at) . ',';
						} elseif ($string[$i] === '\\') {
						...
						if ($string[$i] === '/' && @$string[$i + 1] === '*' && trim($this->selector) == '') {
							...
							foreach ($at_rules as $name => $type) {
								if (!strcasecmp(substr($string, $i + 1, strlen($name)), $name)) {
									...
									if ($type === 'atis') {
										$this->next_selector_at = ($this->next_selector_at?$this->next_selector_at:($this->at?$this->at:LWS_CSSTIDY_DEFAULT_AT));
							...
							$this->status = 'ip';
							if ($this->at == '') {
								$this->at = $this->css_new_media_section($this->at, LWS_CSSTIDY_DEFAULT_AT);
							}
							$this->selector = $this->css_new_selector($this->at,$this->selector);
							$this->_add_token(LWS_CSSTIDY_SEL_START, $this->selector);
							$this->added = false;
						} elseif ($string[$i] === '}') {
							$this->_add_token(LWS_CSSTIDY_AT_END, $this->at);
							$this->at = $this->css_close_media_section($this->at);
							$this->selector = '';
					...
					if ($this->is_token($string, $i)) {
						if (($string[$i] === ':' || $string[$i] === '=') && $this->property != '') {
							...
							if (!$this->get_cfg('discard_invalid_properties') || $this->property_is_valid($this->property)) {
								...
								$this->_add_token(LWS_CSSTIDY_PROPERTY, $this->property);
							}
							...
							$this->status = 'is';
							$this->invalid_at = false;
							$this->_add_token(LWS_CSSTIDY_SEL_END, $this->selector);
							$this->selector = '';
							$this->property = '';
							if ($this->next_selector_at) {
								$this->at = $this->css_close_media_section($this->at);
					...
					if ($this->is_token($string, $i) || $pn) {
						...
						if (($string[$i] === '}' || $string[$i] === ';' || $pn) && !empty($this->selector)) {
							...
							if ((!$this->invalid_at || $this->get_cfg('preserve_css')) && (!$this->get_cfg('discard_invalid_properties') || $valid)) {
								...
								$this->_add_token(LWS_CSSTIDY_VALUE, $this->value);
				...
				case 'ic':
					if ($string[$i] === '*' && $string[$i + 1] === '/') {
						...
						if (strlen($cur_comment) > 1 and strncmp($cur_comment, '!', 1) === 0) {
							$this->_add_token(LWS_CSSTIDY_IMPORTANT_COMMENT, $cur_comment);
							$this->css_add_important_comment($cur_comment);
						}
						else {
							$this->_add_token(LWS_CSSTIDY_COMMENT, $cur_comment);
						}
						$cur_comment = '';
					} else {
	...
	public function css_new_media_section($current_media, $new_media, $at_root = false) {
		...
		if ($current_media
			&& !$at_root
			// numeric $current_media means LWS_CSSTIDY_DEFAULT_AT or inc
